...
model_name = "/insomnia001/home/cd3496/beir/examples/retrieval/training/output/distilbert-base-uncased-hotpotqa-lr1e-5-epochs10-temperature20_full_dev_js_div_full_vector/checkpoint-106250"
#model_name = "Snowflake/snowflake-arctic-embed-m-v1.5_queries_13_to_17"
start_time = time.time()  # Record the start time

# Load the model
model = SentenceTransformer(model_name)
print(f"Loaded model: {model_name}. Loading model to device {device}...")
...
print("Creating MTEB evaluation instance...")
evaluation = MTEB(tasks=[hotpotqa_task])

print('Running evaluation on the full HotpotQA test set...')
# Run the evaluation
# results = evaluation.run(model, verbosity=2, eval_splits=["test"], output_folder=f"results/{model_name}", batch_size=64, corpus_chunk_size=650)
results = evaluation.run(model, verbosity=2, eval_splits=["test"], output_folder=f"results/{model_name}", batch_size=256, corpus_chunk_size=650)
