...
# Create a subset of queries where the length of each query is 15 words
subset_query_ids = [
    key for key, query in hotpotqa_task.queries["test"].items()
    if len(query.split()) <= 10
]

# Print the total number of queries with length <11 words
print(f"Total number of queries with less than equal to 10 words: {len(subset_query_ids)}")
